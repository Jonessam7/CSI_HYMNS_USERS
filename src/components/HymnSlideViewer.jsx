...
const HymnSlideViewer = ({ hymns, initialHymnId, onBack, fontScale = 1, setFontScale }) => {
  ...
  // Build one tab per distinct level actually present among this hymn's
  // variants — every variant has exactly one level now (defaults to Easy).
  const levelTabs = LEVELS
    .map(lv => ({ label: lv, id: variants.find(v => (v.level || "Easy") === lv)?.id }))
    .filter(t => t.id);
